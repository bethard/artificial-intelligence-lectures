...
\tikzset{
  nn bias/.style={fill=gray!50!white,minimum size=2em,font=\small},
  nn io/.style={draw,minimum size=2em,font=\small},
  nn unit/.style={draw,circle split,minimum size=3.25em,font=\small},
  nn edge/.style={draw,thick,-latex},
  nn weight/.style={sloped,anchor=south,near start,font=\footnotesize},
}
\pgfplotsset{
    y noise/.style={
...
\forestset{
  decision/.style={draw,fill=gray,minimum height=1.5em,minimum width=2em,inner sep=0.2em},
  choice/.style={edge label={node[near end,left,inner sep=0.2em]{#1}}},
  result/.style={draw,rounded corners,minimum height=1.5em,minimum width=2em,inner sep=0.2em},
...
\begin{description}[$D_f$]
\item[$D_f$] = \uncover<6->{image regions}
\item[$R_f$] = \uncover<7->{$\{0,1\}$}
\end{description}
\end{block}
\end{column}
\end{columns}
\bigskip
\uncover<8->{Most algorithms work best when $R_f \subseteq \mathbb{R}$ or $|R_f| \ll 100$}
\end{frame}
...
\begin{columns}
\begin{column}<4->{0.48\textwidth}
\uncover<6->{Performance:} \uncover<7->{$\frac{4}{4} = 1.0$}
\end{column}
\begin{column}<4->{0.48\textwidth}
\uncover<6->{Performance:} \uncover<8->{$\frac{3}{4} = 0.75$}
\end{column}
\end{columns}
\end{frame}
...
\begin{frame}[label=knn]{k-Nearest Neighbor Classifiers}
\begin{block}{To classify a new example, $p$}
\begin{itemize}
\item Find the $k$ training examples closest to $p$
\item Classify $p$ with the most common $f(x)$
\end{itemize}
\end{block}
\bigskip
\begin{columns}
\begin{column}{0.2\textwidth}
\LARGE \visible<3->{$k=\alt<6->{3}{1}$}
\end{column}
\begin{column}{0.5\textwidth}
\centering
\begin{tikzpicture}[
  example/.style={draw,circle,inner sep=0em,minimum width=1.25em},
  black/.style={example,fill=black},
  white/.style={example,fill=white},
  neighbor/.style={draw,very thick},
]
\node[black] at (0, 3.5) {};
\node[black] at (3, 3.5) {};
\node[black] at (2, 3) {};
...
\node[black] at (3.5, 2) {};
\only<2->{
\node[example,alt={5}{fill=black}{alt={8}{fill=white}{fill=gray}}] (Q) at (0.5, 1.5) {\only<-4,6-7>{?}};}
\node[white] at (2.5, 2) {};
\node[white] at (3.5, 1) {};
\node[white] (N2) at (1.5, 1) {};
\node[white] (N3) at (0, 0.5) {};
\node[white] at (2.5, 0) {};
\only<4-5,7->{
\path[neighbor] (Q) -- (N1);}
\only<7->{
...
\begin{frame}[label=knn-distance]{Distance Metrics}
\begin{columns}[T]
...
\addplot[only marks,mark options={scale=4,fill=black}] coordinates { (1, 2) };
\addplot[only marks,mark options={scale=4,alt={4-}{fill=black}{fill=gray}},mark=*,nodes near coords={?},every node near coord/.style={anchor=center,visible on={-3}}] coordinates { (3, 2) };
\addplot[only marks,mark options={scale=4,fill=white}] coordinates { (6, 2) };
\only<3->{
\addplot[no marks, domain=1:3] plot {2} node[midway,above] {\footnotesize 2};
\addplot[no marks, domain=3:6] plot {2} node[midway,above] {\footnotesize 3};}
\end{axis}
\end{tikzpicture}
\end{column}
\begin{column}<5->{0.48\textwidth}
...
\addplot[only marks,mark options={scale=4,fill=black}] coordinates { (1, 2) };
\addplot[only marks,mark options={scale=4,alt={7-}{fill=white}{fill=gray}},mark=*,nodes near coords={?},every node near coord/.style={anchor=center,visible on={-6}}] coordinates { (3, 2) };
\addplot[only marks,mark options={scale=4,fill=white}] coordinates { (6, 2) };
\only<6->{
\addplot[no marks, domain=0:1] plot {2*x};
\addplot[no marks, domain=0:3] plot {2*x/3};
\addplot[no marks, domain=0:6] plot {x/3};
\node at (axis cs:2,2) {\footnotesize 0.87};
\node at (axis cs:4.5,2) {\footnotesize 0.96};}
\end{axis}
\end{tikzpicture}
\end{column}
\end{columns}
...
\begin{enumerate}
\item $\bm{\theta}_{a} = [0, 1]$
% L_{0/1} => 2
% L_1 => 6
% L_2 => 20
\item $\bm{\theta}_{b} = [0.6, 1.2]$
% L_{0/1} => 5
% L_1 => 7.2
% L_2 => 12.4
\end{enumerate}
...
\begin{itemize}
\item {\small $\displaystyle L_{0/1}(\theta) = \frac{1}{|X|} \sum_{x \in X} h_{\theta}(x) \neq f(x)$?}
\visible<2->{\\[-0.7em] \hfill $\bm{\theta}_{a}$ $\left( \frac{2}{5} \text{ vs. } \frac{5}{5} \right)$}
\item {\small $\displaystyle L_1(\theta) = \frac{1}{|X|} \sum_{x \in X} \left| f(x) - h_{\theta}(x) \right|$?}
\visible<2->{\\[-0.7em] \hfill $\bm{\theta}_{a}$ $\left( \frac{6}{5} \text{ vs. } \frac{7.2}{5} \right)$}
\item {\small $\displaystyle L_2(\theta) = \frac{1}{|X|} \sum_{x \in X} \left( f(x) - h_{\theta}(x) \right)^2$?}
\visible<2->{\\[-0.7em] \hfill $\bm{\theta}_{b}$ $\left( \frac{20}{5} \text{ vs. } \frac{12.4}{5} \right)$}
\end{itemize}
\end{column}
...
\only<2->{
\addplot[no marks] {x} node[below=0.5em,near end] {$h_{\bm{\theta}_{a}}$};
\addplot[no marks] {0.6 + 1.2*x} node[left,near end] {$h_{\bm{\theta}_{b}}$};
}
\end{axis}
\end{tikzpicture}
\end{column}
...
% L_1 reg => 2 + 3 + 1 => 6 
% L_2 reg => 4 + 9 + 1 => 14
% L_2 loss + 1 / 3 L_1 reg => 1 / 3 + 6 / 3 => 7 / 3
% L_2 loss + 1 / 3 L_2 reg => 1 / 3 + 14 / 3 => 15 / 3
...
% L_1 reg => 3 
% L_2 reg => 9
% L_2 loss + 1 / 3 L_1 reg => 5 / 3 + 3 / 3 => 8 / 3
% L_2 loss + 1 / 3 L_2 reg => 5 / 3 + 9 / 3 => 14 / 3
\end{enumerate}
for the regularization:
\begin{itemize}
\item None?
\visible<2->{$\bm{\theta}_{a}$ $\left( \frac{1}{3} \text{ vs. } \frac{5}{3} \right)$}
\item $L_1$, $\lambda=\frac{1}{3}$?
\visible<2->{$\bm{\theta}_{a}$ $\left( \frac{7}{3} \text{ vs. } \frac{8}{3} \right)$}
\item $L_2$, $\lambda=\frac{1}{3}$?
\visible<2->{$\bm{\theta}_{b}$ $\left( \frac{15}{3} \text{ vs. } \frac{14}{3} \right)$}
\end{itemize}
\visible<2->{
\centering
...
\begin{itemize}
\item \footnotesize\url{http://www.csie.ntu.edu.tw/~cjlin/libsvm/index.html?js=1}
\end{itemize}
}
\end{frame}
...
\begin{frame}[label=nn-neuron]{Neurons in a Neural Network}
\begin{center}
\begin{tikzpicture}[
  align=center,
  nn edge/.style={draw,thick,-latex},
  brace/.style={draw,thick,decoration={brace,mirror},decorate}
]
\node (i1) at (-4, 2) {};
\node (i2)at (-4, 1) {};
\node (i3) at (-4, 0) {};
\node (i4) at (-4, -1) {};
\node (i5) at (-4, -2) {};
\node[draw,ellipse] (unit) at (0, 0) {$\displaystyle
g \left( \sum_{i} \theta_{i} a_i \right)
$};
\node[draw,fill=black,inner sep=0em] (o) at (2.5, 0) {};
\node (o1) at (5, 2) {};
\node (o2)at (5, 1) {};
\node (o3) at (5, 0) {};
\node (o4) at (5, -1) {};
\node (o5) at (5, -2) {};
\path[nn edge] (i1) --  node[above] {$a_1$} (unit);
\path[nn edge] (i2) --  node[above] {$a_2$} (unit);
\path[nn edge] (i3) --  node[above] {$a_3$} (unit);
\path (i4) --  node[above] {\ldots} (unit);
\path[nn edge] (i5) --  node[above] {$a_n$} (unit);
\path[draw,thick] (unit) -- (o);
\path[nn edge] (o) -- (o1);
\path[nn edge] (o) -- (o2);
\path[nn edge] (o) -- (o3);
\path (o) -- node[above] {\ldots} (o4);
\path[nn edge] (o) -- (o5);
\path[brace] (-4, -2) -- node[below] {Input} (-1.75, -2);
\path[brace] (-1.5, -2) -- node[below] {Unit} (1.5, -2);
\path[brace] (1.75, -2) -- node[below] {Output} (5, -2);
\end{tikzpicture}
\end{center}
\pause
Common choices for \textit{activation function} $g$:
\medskip
\begin{columns}[t]
\begin{column}{0.55\textwidth}
$\displaystyle threshold(x) =
\begin{cases}
1 & \text{if } x > 0 \\
0 & \text{otherwise}
\end{cases}$
...
\begin{frame}[label=nn-and]{Neural Network Example: Logical And}
\begin{center}
\begin{tikzpicture}[
  align=center,
]
\node[nn bias] (i0) at (0, 2) {1};
\node[nn io] (i1) at (0, 0) {\only<2-4>{1}\only<5->{0}};
\node[nn io] (i2) at (0, -2) {\only<2->{1}};
\node[nn unit] (and) at (3, 0) {\only<3-4>{0.5}\only<6->{-0.5}\nodepart{lower}$>$0};
\node[nn io] (o) at (6, 0) {\only<4>{1}\only<7->{0}};
\path[nn edge] (i0.east) -- node[nn weight] {-1.5} (and.west);
\path[nn edge] (i1.east) -- node[nn weight] {1.0} (and.west);
\path[nn edge] (i2.east) -- node[nn weight] {1.0} (and.west);
\path[nn edge] (and) -- (o);
\end{tikzpicture}
\end{center}
\end{frame}

\begin{frame}[label=nn-classification]{Neural Network Classification}
\begin{center}
...
\node[nn bias] (i0) at (0, 2.5) {1};
\node[nn io] (i1) at (0, 0) {\only<2->{1}};
\node[nn io] (i2) at (0, -2.5) {\only<2->{1}};
\node[nn bias] (u0) at (4, 2.5) {1};
\node[nn unit] (u1) at (4, 0) {\only<3->{-1.0}\nodepart{lower} $>$0};
\node[nn unit] (u2) at (4, -2.5) {\only<4->{1.5}\nodepart{lower} $>$0};
\node[nn unit] (u3) at (7, 0) {\only<5->{0.5}\nodepart{lower} $>$0};
\node[nn io] (o) at (9, 0) {\only<6->{1.0}};
\path[nn edge] (i0.east) -- node[nn weight] {-1.5} (u1.west);
\path[nn edge] (i0.east) -- node[nn weight] {0.0} (u2.west);
\path[nn edge] (i1.east) -- node[nn weight] {-0.5} (u1.west);
\path[nn edge] (i1.east) -- node[nn weight] {0.5} (u2.west);
\path[nn edge] (i2.east) -- node[nn weight] {1.0} (u1.west);
\path[nn edge] (i2.east) -- node[nn weight] {1.0} (u2.west);
\path[nn edge] (u0.east) -- node[nn weight] {-0.5} (u3.west);
\path[nn edge] (u1.east) -- node[nn weight] {-0.5} (u3.west);
\path[nn edge] (u2.east) -- node[nn weight] {1.0} (u3.west);
\path[nn edge] (u3.east) -- (o.west);
\end{tikzpicture}
\end{center}
\end{frame}

\begin{frame}[label=nn-exercise]{Neural Network Exercise}
\begin{center}
\begin{tikzpicture}[
  align=center,
]
\node[nn bias] (b1) at (0, 3) {1};
\node[nn bias] (b2) at (3, 3) {1};
\node[nn bias] (b3) at (6, 3) {1};
\node[nn io] (i1) at (0, 0) {0};
\node[nn io] (i2) at (0, -3) {1};
\node[nn unit] (u1) at (3, 0) {\only<2->{-1.0}\nodepart{lower}$>$0};
\node[nn unit] (u2) at (3, -3) {\only<2->{0.5}\nodepart{lower}$>$0};
\node[nn unit] (u3) at (6, 0) {\only<2->{2.0}\nodepart{lower}$>$0};
\node[nn unit] (u4) at (6, -3) {\only<2->{-0.5}\nodepart{lower}$>$0};
\node[nn unit] (u5) at (8.5, 0) {\only<2->{-1.0}\nodepart{lower}$>$0};
\node[nn io] (o) at (10.5, 0) {\only<2->{0.0}};
\path[nn edge] (b1) -- node[nn weight] {-0.5} (u1);
\path[nn edge] (b1) -- node[nn weight] {-0.5} (u2);
\path[nn edge] (b2) -- node[nn weight] {0.5} (u3);
\path[nn edge] (b2) -- node[nn weight] {0.5} (u4);
\path[nn edge] (b3) -- node[nn weight] {-0.5} (u5);
\path[nn edge] (i1) -- node[nn weight] {1.0} (u1);
\path[nn edge] (i1) -- node[nn weight] {0.5} (u2);
\path[nn edge] (i2) -- node[nn weight] {-0.5} (u1);
\path[nn edge] (i2) -- node[nn weight] {1.0} (u2);
\path[nn edge] (u1) -- node[nn weight] {1.0} (u3);
\path[nn edge] (u1) -- node[nn weight] {0.5} (u4);
\path[nn edge] (u2) -- node[nn weight] {1.5} (u3);
\path[nn edge] (u2) -- node[nn weight] {-1.0} (u4);
\path[nn edge] (u3) -- node[nn weight] {-0.5} (u5);
\path[nn edge] (u4) -- node[nn weight] {1.0} (u5);
\path[nn edge] (u5) -- (o);
\end{tikzpicture}
\end{center}
\end{frame}
