...
Optimal?   & \uncover<3->{Yes, for optimal opponent} \uncover<4->{\alert<4->{and suboptimal!}} \\
Time? & \uncover<5->{$O(b^m)$, all nodes in the tree} \\
Space? & \uncover<6->{$O(bm)$, like depth first search} \\
\end{tabular}
\end{frame}
