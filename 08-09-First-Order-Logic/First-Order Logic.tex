...
\begin{frame}{Truth in First Order Logic}
	\begin{block}{Key Ideas}
		\begin{itemize}
			...
			\item $\textit{R}(\textit{Term}_1, \ldots, \textit{Term}_n)$ is true iff
			      $\langle\textit{Term}_1, \ldots, \textit{Term}_n\rangle \in \textit{R}$
		\end{itemize}
	\end{block}
	\begin{block}{Example}
		Given the relation: \\
		\tab$\textit{SquareRoot} = \{
				...
				\ldots \}$ \\
		\pause
		Then $\textit{SquareRoot}(4, 2)$ is \pause \alert{true} because \\
		\tab$\langle 4, 2 \rangle \in \textit{SquareRoot}$ \\
		\pause
		And $\textit{SquareRoot}(2, 2)$ is \pause \alert{false} because \\
		\tab$\langle 2, 2 \rangle \not\in \textit{SquareRoot}$
	\end{block}
\end{frame}
...
\begin{frame}{Universal Quantification ($\forall$)}
	...
	\begin{block}{Example}
		...
		\tab\tab$\textit{Bird}(\textit{Swallow}) \limpl \textit{LaysEggs}(\textit{Swallow})$ is true \\
		\tab\tab$\textit{Bird}(\textit{Emu}) \limpl \textit{LaysEggs}(\textit{Emu})$ is true \\
		\tab\tab$\textit{Bird}(\textit{Badger}) \limpl \textit{LaysEggs}(\textit{Badger})$ is true \\
		\tab\tab\ldots \\[.5em]
		\pause
		But $\forall x\ \textit{LaysEggs}(x) \limpl \textit{Bird}(x)$ is false because: \\
		\pause
		\tab\tab$\textit{LaysEggs}(\textit{Platypus}) \limpl \textit{Bird}(\textit{Platypus})$ is false
	\end{block}
\end{frame}
\begin{frame}{Universal Quantification in Translation}
	\begin{block}{Common Mistake}
		What does this mean? \\
		\tab\tab$\forall x\ \textit{Bird}(x) \land \textit{LaysEggs}(x)$ \\
		\pause
		Answer: \\
		\tab\tab\emph{Everything is a bird and everything lays eggs} \\
		Intended statement: \\
		\tab\tab$\forall x\ \textit{Bird}(x) \limpl \textit{LaysEggs}(x)$ \\
	\end{block}
	\pause
...
\begin{frame}{Existential Quantification}
	...
	\begin{block}{Example}
		...
		\pause
		\tab\tab$\textit{Mammal}(\textit{Platypus}) \land \textit{LaysEggs}(\textit{Platypus})$ is true \\[.5em]
		\pause
		But $\exists x\ \textit{Mammal}(x) \land \lnot\textit{Animal}(x)$ is false because: \\
		\pause
		\tab\tab$\textit{Mammal}(\textit{Person}) \land \lnot\textit{Animal}(\textit{Person})$ is false \\
		\tab\tab$\textit{Mammal}(\textit{Platypus}) \land \lnot\textit{Animal}(\textit{Platypus})$ is false \\
		\tab\tab$\textit{Mammal}(\textit{Spam}) \land \lnot\textit{Animal}(\textit{Spam})$ is false \\
		\tab\tab\ldots \\
	\end{block}
\end{frame}
\begin{frame}{Existential Quantification in Translation}
	\begin{block}{Common Mistake}
		What does this mean? \\
		\tab\tab$\exists x\ \textit{Mammal}(x) \limpl \textit{LaysEggs}(x)$ \\
		\pause
		Answer: \\
		\tab\tab\emph{There is something that is not a mammal or lays eggs} \\
		Intended statement: \\
		\tab\tab$\exists x\ \textit{Mammal}(x) \land \textit{LaysEggs}(x)$ \\
	\end{block}
	\pause
	\begin{block}{Rule of Thumb}
...
\begin{frame}{Animal Kingdom Translations}
	\begin{itemize}
		\item
			...
			\pause
			$\forall x\ \textit{Horse}(x) \limpl \textit{Mammal}(x)$
		\pause
		\item
			All birds have wings \\
			\pause
			$\forall x\ \textit{Bird}(x) \limpl \textit{HasWings}(x)$
		\pause
		\item
		...
		\item
			...
			\pause
			$\exists x\ \textit{Bird}(x) \land \lnot\textit{Flies}(x)$
		\pause
		\item
			Animals that fly have wings \\
			\pause
			$\forall x\ \textit{Animal}(x) \land \textit{Flies}(x) \limpl \textit{HasWings}(x)$
		\pause
		\item
			Not all swimming animals have fins \\
			\pause
			$\lnot\forall x\ \textit{Animal}(x) \land \textit{Swim}(x) \limpl \textit{HasFins}(x)$
			\pause
...
\begin{frame}{Wumpus World Translations}
	\begin{block}{Diagnostic Rules}
		...
		\tab\tab$\forall y\ \textit{Breezy}(y) \limpl (\exists x\ \textit{Pit}(x) \land \textit{Adjacent}(x, y))$
	\end{block}
	\pause
	\begin{block}{Causal Rules}
		From cause, determine effect: \\
		\tab\tab$\forall x,y\ (\textit{Pit}(x) \land \textit{Adjacent}(x, y)) \limpl \textit{Breezy}(y)$
	\end{block}
	\pause
	\begin{block}{Definition Rules}
		Bidirectional: \\
		\tab\tab$\forall y\ \textit{Breezy}(y) \liff (\exists x\ \textit{Pit}(x) \land \textit{Adjacent}(x, y))$
	\end{block}
\end{frame}
...
\begin{frame}{Reduction to Propositional Logic}
	...
	\begin{block}{Propositional Logic}
		$
		\begin{array}{l}
			\pause\textit{Mammal}(\textit{EggLayer}) \land \textit{LaysEggs}(\textit{EggLayer}) \\
			\pause\textit{Mammal}(\textit{Platypus}) \limpl \textit{WarmBlooded}(\textit{Platypus}) \\
			\textit{Mammal}(\textit{Crocodile}) \limpl \textit{WarmBlooded}(\textit{Crocodile}) \\
			\pause\textit{Mammal}(\textit{Platypus}) \\
			\pause\lnot \textit{WarmBlooded}(\textit{Crocodile})
		\end{array}
		$
	\end{block}
\end{frame}
...
\begin{frame}{Unification}
	...
	\end{block}
	\pause
	\begin{block}{Example: $\textit{Eats}(\textit{Panda}, y)$?}
		\footnotesize
		$
		\begin{array}{@{}l@{\hspace{.2em}}l@{\hspace{.2em}}l@{}}
			p                              & q                                  &
				\theta \\
			\hline
			\pause\textit{Eats}(\textit{Panda}, y) & \textit{Eats}(\textit{Panda}, \textit{Leaves}) &
				\pause\{y\!\!=\!\!\textit{Leaves}\} \\
			\pause\textit{Eats}(\textit{Panda}, y) & \textit{Eats}(x, \textit{Pizza})           &
				\pause\{x\!\!=\!\!\textit{Panda}, y\!\!=\!\!\textit{Pizza}\} \\
			\pause\textit{Eats}(\textit{Panda}, y) & \textit{Eats}(x, \textit{FavFood}(x))      &
				\pause\{x\!\!=\!\!\textit{Panda}, y\!\!=\!\!\textit{FavFood}(\textit{Panda})\}
		\end{array}
		$
	\end{block}
\end{frame}
