...
\item[Sound?]
\uncover<3->{Yes, uses Generalized Modus Ponens}
\item[Complete?]
...
\begin{frame}[label=backwards-chaining-backtracking]{Backwards Chaining Backtracking}
\small
\extrarowheight=.25em
$
\begin{array}{ @{} l l @{} }
\mbox{Prove:} & \textit{Connected}(A, D) \\
\mbox{Given:}
...
\item[Complete?]
\uncover<3->{
No, if only binary resolution is used \\
Yes, with subset resolution or factoring}
\end{description}
\bigskip
Retrieving existential goals may require adding $\textit{Answer}(x)$
\end{block}
\end{frame}
