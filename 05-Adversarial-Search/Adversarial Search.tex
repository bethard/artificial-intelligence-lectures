...
\begin{frame}[fragile]{Minimax Code}
	\begin{semiverbatim}\scriptsize\bfseries
		...
		\pause
		\keyword{def} \defn{\alt<7>{\alert{min}}{max}_value_action}(problem, state):
		    ...
		    \keyword{return} \alt<7>{\alert{min}}{max}(items)
	\end{semiverbatim}
\end{frame}
...
\begin{frame}{Minimax Properties}
	...
	\end{center}
	\begin{tabular}{ll}
		Complete? \pause & Yes, if tree is finite \pause \\
		Optimal?  \pause & Yes, for optimal opponent \pause \alert{and suboptimal!} \pause \\
		Time?     \pause & $O(b^m)$, all nodes in the tree \pause \\
		Space?    \pause & $O(bm)$, like depth first search \\
	\end{tabular}
\end{frame}
