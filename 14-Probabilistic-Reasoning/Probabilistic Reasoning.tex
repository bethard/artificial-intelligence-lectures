...
\begin{frame}[t,label=factor-inference]{Exact Inference via Factors}
\small
\setlength{\arraycolsep}{0.2em}
$
\begin{array}{ @{} l l l @{} }
\lefteqn{P(B|j,m) = \displaystyle \alpha P(B)\sum_{e' \in E} P(e') \sum_{a' \in A} P(a'|B,e')P(j|a')P(m|a')} \\[2em]
\only<2>{
& = & \displaystyle 
\alpha
\scriptfactor{ r }{
