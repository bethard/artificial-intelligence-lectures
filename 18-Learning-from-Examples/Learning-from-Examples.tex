...
\forestset{
  decision/.style={draw,fill=gray,minimum height=2em,minimum width=2em,inner sep=0.1em},
  choice/.style={edge label={node[near end,left,inner sep=0.1em]{#1}}},
  result/.style={draw,rounded corners,minimum height=2em,minimum width=2em,inner sep=0.1em},
  decision tree/.style={
    for tree={
      l sep=3em,
      edge={thick,-latex},
      % square edges: http://tex.stackexchange.com/questions/108710/square-edges-in-forest-package
      edge path={\noexpand\path[\forestoption{edge}] (\forestOve{\forestove{@parent}}{name}.parent anchor) -- +(0,-2em)-| (\forestove{name}.child anchor)\forestoption{edge label};},
    }
  },
}
...
\small
\begin{forest}
decision tree,for tree={s sep=2em}
[{Weather},decision
...
\small
\begin{forest}
decision tree,for tree={s sep=1.5em}
[X,decision
  [Y,decision,choice={true}
    [false,result,choice={true}]
    [true,result,choice={false}]
  ]
  [Y,decision,choice={false}
...
decision tree,for tree={s sep=1.5em}
[A,decision
  [B,decision,choice={true}
    [true,result,choice={true}]
    [C,decision,choice={false}
      [false,result,choice={true}]
      [true,result,choice={false}]
    ]
  ]
