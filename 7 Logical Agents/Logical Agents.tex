...
\begin{frame}{Knowledge Bases}
	\begin{block}{Idea: Separate Knowledge from Reasoning}
		...
		\begin{tabular}{|l|l}
			\hhline{-~}
			\parbox[c][0.4in]{1.25in}{Inference Engine} & $\leftarrow$ domain-independent algorithms \\
			\hhline{-~}
			\parbox[c][0.4in]{1.25in}{Knowledge Base}   & $\leftarrow$ domain-specific content \\
			\hhline{-~}
		\end{tabular}
	\end{block}
	\pause
	\begin{block}{Knowledge Base (KB) Properties}
		\begin{itemize}
			...
			\item Can \SC{Tell} it new ``sentences''
			\item Can \SC{Ask} it ``queries''
		\end{itemize}
	\end{block}
\end{frame}
\begin{frame}[fragile]{Knowledge-Based Agents}
	\begin{semiverbatim}\bfseries\scriptsize
		\key{class} \defn{KnowledgeBaseAgent}(object):\pause
		    \key{def} \defn{__init__}(\key{self}, knowledge_base):
		        \key{self}.knowledge_base = knowledge_base
		        \key{self}.time = 0
		    \key{def} \defn{take_action}(\key{self}, percept):
		        \pause\cmnt{# convert the percept to knowledge}
		        percept_sentence = \key{self}.percept_to_sentence(percept)
		        \key{self}.knowledge_base.tell(percept_sentence)
		        \pause\cmnt{# select an action based on the knowledge}
		        action_query = \key{self}.make_action_query()
		        action = \key{self}.knowledge_base.ask(action_query)
		        \pause\cmnt{# update the knowledge base with the planned action}
		        action_sentence = \key{self}.action_to_sentence(action)
		        \key{self}.knowledge_base.tell(action)
		        \key{self}.time += 1
...
\begin{frame}{Wumpus World Description}
	\begin{block}{Environment}
		\begin{itemize}
			...
			\item \SC{Shoot} uses the arrow and kills wumpus if facing it
			\item Gold glitters and \SC{Grab} picks it up if in same square
		\end{itemize}
	\end{block}
	\pause
	\begin{block}{Actions}
		\SC{Turn-Left}, \SC{Turn-Right}, \SC{Forward}, \SC{Grab}, \SC{Shoot}
	\end{block}
	\pause
	\begin{block}{Score}
		\begin{itemize}
...
\begin{frame}{Wumpus World Properties}
	\begin{tabular}{ll}
		\key{Observable?}    \pause & No, only local perception \pause \\
		\key{Deterministic?} \pause & Yes, state+action determines outcome \pause \\
		\key{Episodic?}      \pause & No, involves a sequence of actions \pause \\
		\key{Static?}        \pause & Yes, pits and wumpus are stationary \pause \\
		\key{Discrete?}      \pause & Yes, no real-valued states or actions \pause \\
		\key{Single-Agent?}  \pause & Yes, wumpus takes no (real) actions
	\end{tabular}
\end{frame}


\subsection{Wumpus World Examples}
\begin{frame}{Exploring a Wumpus World}
	\begin{center}
		\arrayrulewidth=2pt
		\begin{tabular}{|l|l|l|l|}
			...
			\hline
			\wumpcell{}{\uncover<5-10>{P?}}{\only<11->{\Huge P}} &
			\wumpcell{}{\uncover<14->{OK}}{} &
			\emptycell &
			\emptycell \\
			\hline
			\wumpcell{\uncover<4->{B}}{\uncover<2->{OK}}{\only<3-5>{\fbox{A}}} &
			\wumpcell{}{\uncover<5->{\alt<-9>{P?}{OK}}}{\only<12-14>{\fbox{A}}} &
			\wumpcell{\uncover<16->{SG}}{\uncover<14->{OK}}{\only<15->{\fbox{A}}} &
			\emptycell \\
			\hline
			\wumpcell{}{\uncover<2->{OK}}{\only<-2,6>{\fbox{A}}} &
			\wumpcell{\uncover<8->{S}}{\uncover<2->{OK}}{\only<7-11>{\fbox{A}}} &
			\wumpcell{}{}{\uncover<9->{\Huge W}} &
			\emptycell \\
			\hline
		\end{tabular}
	\end{center}
...
\begin{frame}{Wumpus Entailment Example}
	\begin{columns}
		\begin{column}{3.25in}
			\only<1>{\includegraphics[height=2.25in]{wumpus-models1}}%
			\only<2>{\includegraphics[height=2.25in]{wumpus-models2}}%
			\only<3-4>{\includegraphics[height=2.25in]{wumpus-models3}}%
			\only<5-6>{\includegraphics[height=2.25in]{wumpus-models4}}%
		\end{column}
		\begin{column}{1in}
...
\begin{frame}{Evaluating Complex Expressions}
	\begin{block}{Given}
		\begin{tabular}{lll}
			$P=\EM{true}$ & $Q=\EM{false}$ & $R=\EM{true}$ 
		\end{tabular}
	\end{block}
	...
	\end{block}
	\begin{enumerate}
		\pause\item $\EM{true} \lor \EM{true} \limpl \lnot (\EM{false} \land \lnot \EM{true})$
		\pause\item $\EM{true} \lor \EM{true} \limpl \lnot (\EM{false} \land \EM{false})$
		\pause\item $\EM{true} \lor \EM{true} \limpl \lnot \EM{false}$
		\pause\item $\EM{true} \lor \EM{true} \limpl \EM{true}$
		\pause\item $\EM{true} \limpl \EM{true}$
		\pause\item $\EM{true}$
	\end{enumerate}
\end{frame}
...
\begin{frame}[fragile]{Truth Table Inference Code}
	\begin{semiverbatim}\scriptsize\bfseries
		\key{def} \defn{truth_table_entails}(knowledge_base, query):\pause
		    ...
		    \key{return} all(
		        query_true(assignment) \key{if} kb_true(assignment) \key{else} \key{True}
		        \key{for} assignment \key{in} all_models(symbols))
		
		\pause\key{def} \defn{all_models}(symbols):
		    \pause\cmnt{# base case: no symbols, generate an empty assignment}
		    \key{if} \key{not} symbols:
		        \key{yield} {}
		    \pause\cmnt{# recursive case: assign to the first symbol and recurse}
		    \key{else}:
		        first, rest = symbols[0], symbols[1:]
		        \key{for} assignment \key{in} all_models(rest):
		            \key{for} value \key{in} [True, False]:
...
\begin{frame}{Truth Table Inference Properties}
	...
	\begin{block}{Truth Table Inference Properties}
		\begin{tabular}{@{}l@{\hspace{.3em}}l}
			\pause\key{Sound?}    & \pause Yes, directly implements entailment definition \\
			\pause\key{Complete?} & \pause Yes, explores all possibilities \\
			\pause\key{Time?}     & \pause Given $n$ symbols, takes $O(2^n)$ \\
		\end{tabular}
	\end{block}
\end{frame}
...
\begin{frame}[fragile]{Forward Chaining Code}
	\begin{semiverbatim}\bfseries\scriptsize
		\key{def} \defn{forward_chaining_entails}(knowledge_base, query):
		    \pause\cmnt{# count the symbols in the body of each clause}
		    counts = \{\}
		    \key{for} clause \key{in} knowledge_base.get_clauses():
		    ...
		    \key{while} agenda:
		        symbol = agenda.pop()
		        \key{if} symbol \key{not in} inferred:
		            inferred.add(symbol)
		            \pause\cmnt{# update counts and infer heads when possible}
		            \key{for} clause \key{in} knowledge_base.get_clauses():
		                \key{if} symbol \key{in} clause.body:
		                    counts[clause] -= 1
		                    \pause\key{if} counts[clause] == 0:
		                        \key{if} clause.head == query:
		                            \key{return} \key{True}
		                        agenda.append(clause.head)
		    \key{return} \key{False}
	\end{semiverbatim}
\end{frame}
...
\begin{frame}{Forward Chaining Properties}
	\begin{block}{Properties}
		\begin{tabular}{ll}
			\pause\key{Sound?}    & \pause Yes, uses Modus Ponens \\
			\pause\key{Complete?} & \pause Yes (more on this in a moment) \\
			\pause\key{Time?}     & \pause $O(n)$ given $n$ statements in KB \\
		\end{tabular}
	\end{block}
	\pause
...
\begin{frame}{Forward Chaining Completeness}
	\begin{block}{Prove: If $\EM{KB} \models Q$ then $\EM{KB} \vdash_{\EM{\scriptsize forward-chaining}} Q$}
		\begin{enumerate}
			\item<2-> $\EM{KB} \models Q$, so $Q$ is \emph{true} in every model of \emph{KB}
			\item<2-> Final \texttt{inferred} set (not inferred = \emph{false}) is a model 
			\item<4-> So $Q$ is true in the \texttt{inferred} model
			\item<4-> So $\EM{KB} \vdash_{\EM{\scriptsize forward-chaining}} Q$
		\end{enumerate}
	\end{block}
	\begin{block}<3->{Prove: The final \texttt{inferred} set is a model of $\EM{KB}$}
		\begin{enumerate}
...
\begin{frame}{Resolution (Again)}
	\begin{block}{Example}
		$
		\begin{array}{l}
		\lnot \EM{IsSnowing} \lor \EM{IsCold} \\
		\lnot \EM{IsCold} \lor \EM{WearCoat} \\
		\hline
		\lnot \EM{IsSnowing} \lor \EM{WearCoat}
		\end{array}
		$
	\end{block}
	\begin{block}{Intuition}
		\begin{itemize}
			\item If \EM{IsCold} is \emph{true}, then \EM{WearCoat} must be \emph{true}
			\item If \EM{IsCold} is \emph{false}, then \EM{IsSnowing} must be \emph{false}
			\item So either \EM{IsSnowing} is \emph{false} or \EM{WearCoat} is \emph{true}
		\end{itemize}
	\end{block}
\end{frame}
\begin{frame}{Resolution}
	...
	\begin{block}{Inference by Resolution}
		\begin{itemize}
			...
			\item Assume $\EM{KB} \land \lnot\alpha$ (a.k.a $\lnot (\EM{KB} \limpl \alpha)$)
...
\begin{frame}{Resolution Properties}
	\begin{block}{Properties}
		\begin{tabular}{@{}l@{\hspace{.5em}}l}
			\key{Sound?}    & Yes, uses Resolution \\
			\key{Complete?} & Yes, proof similar to Forward Chaining proof \\
			\key{Time?}     & Worst case exponential in \# of symbols \\
		\end{tabular}
	\end{block}
	\pause
	\begin{block}{Notes}
...
\begin{frame}{WalkSAT}
	\begin{block}{Key Ideas}
		\begin{itemize}
			...
			\item Choice of symbol to flip is sometimes random, sometimes by \SC{Min-Conflicts}
			\item Quit after a certain number of steps
		\end{itemize}
	\end{block}
	\begin{block}{Properties}
		\begin{tabular}{ll}
			\pause\key{Sound?}    & \pause Yes, only returns when KB is \emph{true} \\
			\pause\key{Complete?} & \pause No, could stop early  \\
			\pause\key{Time?}     & \pause Can be quite fast \\
		\end{tabular}
	\end{block}
\end{frame}
...
\begin{frame}{Key Points}
	...
	\begin{block}{Inference}
		\begin{itemize}
			...
			\item Local search, e.g. \SC{WalkSAT}
		\end{itemize}
	\end{block}
\end{frame}
